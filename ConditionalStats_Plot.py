...
class ConditionalStats_Plot:
    ...
    def readfile(self,rpm_coflow,otsu_fact,criteria):
        ...
        try:
            ...
            strng = "rpm" + str(rpm_coflow) + criteria_map[criteria]+"_otsuby" + str(
                otsu_fact) + "_numimgs1500_normalisedminsubdetectioncriteria_dx_199pts_win5"
    ...
    def velocity_transform_coords_2d(self,U,V,slope):
        theta = np.arctan(slope) # angle of edge
        #neg_ind = np.where(theta<0)[0]
        #theta[neg_ind] = theta[neg_ind]+np.pi
    ...
    def data_extract(self,loc_mm,h_win,dx,dy):
        ...
        u_rms = np.transpose(np.sqrt(np.mean((np.transpose(U[:,:,int(shp_arr[-1]/2)])-U_mean)**2.0,axis=0)))
    ...
    def read_ref_data(self):
        ...
        with open(file_path, newline='') as csvfile:
            ...
            U_Watanabe2014 = []
            for row in csv_reader:
                x_loc4.append(float(row['x'].replace(',', '.')) * 0.05)
    ...
    def extract_data_compare(self):
        ...
        key_list = [0,680]#,0,0,680,680]#,680,680]#[0,680,0,680]#[680,680,680,680]#[0,0,0,0,680,680]#,0,680,680,680]#[0,0,0,0]#,0,0,0]#,375,680]#,375]
        otsu_list = [10,10]#,5,10,5,10]#,5,10]#[10,10,10,10]#[10,8,6,4,6,4]#[10,8,6,4,6,4,1]#[10,8,6,4]#,8,10,50]
        ...
        for key_ind in range(len(key_list)):
            ...
            for i in range(len(xloc)):
                ...
                enstrophy_plot, vorticity_plot, vorticity_mod_plot, enstrophy_flux_plot = self.data_extract(ind, h_win, dx, dy)
                ...
                denom_fact_ke =((Ucenter - ucoflow) ** 3.0)/np.mean(self.y_half[xloc_avg])#(Ucenter) ** 3.0#
                ...
                if vorticity_plot_opt == 'y':
                    ...
                    if ke_calc=='y':
                        ...
                        ax11.scatter(xplot, -vorticity_plot*np.mean(self.y_half[xloc_avg])/denom_fact, s=mrkr_size)
        ...
        namesave='vorticity2rpm0and680_hwin'+str(h_win)+'_xloc'+str(xloc[0])+'_'+self.axial_location+'D'
